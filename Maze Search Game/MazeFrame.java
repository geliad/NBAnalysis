...
public class MazeFrame extends JFrame {
	 ...
	 * 
	 * @param mazeData
	 *            for details about what should be in this array see class comments
	 *            in Maze.java
	 * @param entryLoc
	 *            the entry location of the maze
	 * @param exitLoc
	 *            the "exit" location of the maze PRE: 0 <= entryLoc.getRow() <
	 *            mazeData.length and 0 <= entryLoc.getCol() < mazeData[0].length
	 *            and 0 <= exitLoc.getRow() < mazeData.length and 0 <=
	 *            exitLoc.getCol() < mazeData[0].length
	 */
	...
	/**
	 * getSearchMessage returns the message to display for a successful or failed
	 * search.
	 * 
	 * @param success
	...
	class MazeKeyListener extends KeyAdapter { // inner class -- has access to outer
		...
		/**
		 * keyPressed is called when the user types a character. The action taken is to
		 * do the maze search, then update the display according to the results of the
		 * search.
		 * 
		 * @param event
